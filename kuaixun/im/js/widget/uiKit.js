			...
			SessionList.prototype.update = function (data) {
				...
				if (sessions.length === 0) {
					...
					for (i = 0; i < sessions.length; i++) {
						...
						str = ['<li class="panel_item ' + (info.crtSession === info.target ? 'active' : '') + '" data-scene="' + info.scene + '" data-account="' + info.account + '"' + `onclick="getTeamMemberInfoAgain('${info.account}','${info.scene}')"` + '>',
				...
				if (teams && teams.length > 0) {
					for (var i = 0, l = teams.length; i < l; ++i) {
						...
						if (info.type === 'normal') {
							...
							flag2 = true;
							tmp2 += ['<li class="panel_item ' + (info.crtSession === info.target ? 'active' : '') + '" data-gtype="advanced" data-type="team" data-account="' + info.teamId + '"' + `onclick="getTeamMemberInfoAgain('${info.teamId}','team')"` + '>',
								'<div class="panel_avatar"><img class="panel_image" src="' + info.avatar + '"/></div>',
								'<div class="panel_text">',
								'<p class="panel_single-row">' + info.nick + '</p>',
								'</div>',
